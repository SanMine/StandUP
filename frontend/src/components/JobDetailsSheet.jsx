...
const JobDetailsSheet = ({
    ...
    // Fetch full job details when sheet opens
    useEffect(() => {
        let canceled = false;
        ...
        const fetchJobDetails = async () => {
            ...
            try {
                ...
                if (!canceled && res && res.success) {
                    setJob(res.data);
                    setIsSaved(Array.isArray(savedJobIds) && savedJobIds.includes(res.data.id));
                }
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const checkIfApplied = async () => {
            ...
            try {
                ...
                if (found && !canceled) {
                    setJob(prev => ({ ...(prev || {}), applicationStatus: found.status || 'applied' }));
                }
            } catch (err) {
    ...
    const companyName = job?.company || job?.employer?.company_name || job?.employer?.name || '';
    const title = job?.title || job?.role || 'Untitled Role';
    const score = job?.matchPercentage ?? job?.matchScore ?? job?.match_score ?? 0;
                ...
                {loading ? (
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            <Loader2 className="w-8 h-8 mx-auto mb-4 animate-spin text-[#FF7000]" />
                            <p className="text-[#4B5563]">Loading job details...</p>
                            {isPremium && (
                                <p className="text-sm text-[#9CA3AF] mt-2">Calculating AI match analysis...</p>
                            )}
                        </div>
                    </div>
                ) : !job ? (
                    <div className="flex items-center justify-center h-full">
                        <p className="text-[#4B5563]">Job not found</p>
                    </div>
                ) : (
                    <>
                        <SheetHeader>
                            <div className="flex items-start gap-4 mb-4">
                                <img
                                    src={job?.logo || job?.company_logo || '/images/company-placeholder.png'}
                        ...
                        <div className="pb-8 mt-6 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex items-center gap-2 text-sm">
                                    ...
                                    <span className="text-[#4B5563]">{job?.location || '—'}</span>
                                </div>
                                <div className="flex items-center gap-2 text-sm">
                                    <Briefcase className="h-4 w-4 text-[#4B5563]" />
                                    <span className="text-[#4B5563]">{job?.type || '—'}</span>
                                </div>
                                <div className="flex items-center gap-2 text-sm">
                                    <Building className="h-4 w-4 text-[#4B5563]" />
                                    <span className="text-[#4B5563]">{job?.mode || '—'}</span>
                                </div>
                                <div className="flex items-center gap-2 text-sm">
                                    <DollarSign className="h-4 w-4 text-[#4B5563]" />
                                    <span className="text-[#4B5563]">{job?.salary || '—'}</span>
                                </div>
                            </div>
                            ...
                            {/* AI analysis */}
                            {isPremium ? (
                                (job?.strongMatchFacts || job?.areasToImprove) && (
                                    <div className="bg-gradient-to-br from-[#FFFDFA] to-[#FFF7ED] rounded-lg p-5 border border-orange-100">
                                        ...
                                        <div className="space-y-4">
                                            {Array.isArray(job?.strongMatchFacts) && job.strongMatchFacts.length > 0 && (
                                                <div>
                                                    ...
                                                    <ul className="space-y-2">
                                                        {job.strongMatchFacts.map((reason, idx) => (
                                            ...
                                            {Array.isArray(job?.areasToImprove) && job.areasToImprove.length > 0 && (
                                                <div>
                                                    ...
                                                    <ul className="space-y-2">
                                                        {job.areasToImprove.map((reason, idx) => (
                                                            <li key={idx} className="flex items-start gap-2 text-sm text-[#1F2937]">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-[#4B5563] leading-relaxed">{job?.description || job?.summary || ''}</p>
                            </div>
                            ...
                            {/* Culture */}
                            {Array.isArray(job?.culture) && job.culture.length > 0 && (
                                <div>
                                    <h3 className="font-semibold text-[#0F151D] mb-3">Company Culture</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {job.culture.map((tag, idx) => (
                            ...
                            <div className="flex gap-3 pt-4 border-t">
                                ...
                                >
                                    {job?.applicationStatus === 'applied' ? 'Applied' : (applying ? 'Applying...' : 'Apply Now')}
                                </Button>
